...
class SiteSettingsForm(forms.ModelForm):
    ...
    def _email_config_from_cleaned(self, cleaned) -> EmailConfig:
        return EmailConfig(
    ...
    def clean(self):
        ...
        if not host or not host_user or not password or is_placeholder(password):
            raise ValidationError("Google SMTP host, username, and app password are required.")

        config = self._email_config_from_cleaned(cleaned)
